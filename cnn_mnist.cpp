                        ...
                        for (int j=0; j<28; j++) {
                                ...
                                for (int k=0; k<filter_size; k++) {
                                        for (int l=0; l<filter_size; l++) {
                                                int left = int(img[i+k+1][j+l-2]);
                                                int right = int(conv_w[filter_dim][k][l] * pow(2, 10));
                                                product += left * right;
                                                // conv_layer[filter_dim][i][j] += img[i+k+1][j+l-2]*conv_w[filter_dim][k][l];
                                        }
                                }
                                conv_layer[filter_dim][i][j] = (double) product / pow(2, 10);
                                // Convert char and doubles into int
                                // Verilog input: img[i+k+1][j+l-2], conv_w[filter_dim][k][l]
                                // Verilog output: conv_layer[filter_dim][i][j]
