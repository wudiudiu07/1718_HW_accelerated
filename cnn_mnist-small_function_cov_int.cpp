...
double get_weight(int a, int i) {
  return double(conv_w[a][i]) / 1024.0;
}
void get_convb(double cb[5][28][28]) {
  cb = conv_b;
}

int convolution(int a, int b, unsigned char img[], int offset_img_a, int offset_img_b, int weight[49]) {
  int result = 0;
  for (int i = 0; i < a; i++) {
    for (int j = 0; j < b; j++) {
...
int set_sig_layer(int u, int f_dim, int i, int j) {
...
int initialise_weights() {
  for (int i = 0; i < 5; i++) {
    for (int j = 0; j < 28; j++) {
      for (int k = 0; k < 28; k++) {
        if (j < 7 && k < 7) {
          conv_w[i][j * 7 + k] = 1024 * (2 * double(rand()) / RAND_MAX - 1);
        }
  ...
  for (int filter_dim = 0; filter_dim < 5; filter_dim++) {
    for (int i = 0; i < 28; i++) {
      for (int j = 0; j < 28; j++) {
        ...
        conv_layer[filter_dim][i][j] = double(convolution(filter_size, filter_size, img, i + 1, j - 2, conv_w[filter_dim])) / 1024.0;
...
int update_weights(int a) {
  ...
  for (int i = 0; i < 5; i++) {
    for (int k = 0; k < 7; k++) {
      for (int j = 0; j < 7; j++) {
        conv_w[i][k * 7 + j] -= 1024 * eta * dw_conv[i][k][j];
      }
    }
    for (int l = 0; l < 28; l++) {
      for (int m = 0; m < 28; m++) {
...
int backward_pass(int a) {
        ...
        for (int l = 0; l < 2; l++) {
          for (int m = 0; m < 2; m++) {
            if (max_pooling[filter_dim][i + l][j + m] == 1)
              dw_max[filter_dim][i * 28 + j] = 1024 * delta2[k];
            else
  ...
  for (int filter_dim = 0; filter_dim < 5; filter_dim++) {
    for (int i = 0; i < 28; i++) {
      for (int j = 0; j < 28; j++) {
        db_conv[filter_dim][i][j] = double(dw_max[filter_dim][i * 28 + j]) / 1024.0;
      }
    }
  }
  ...
  for (int filter_dim = 0; filter_dim < 5; filter_dim++) {
    for (int k = 0; k < 7; k++) {
      for (int l = 0; l < 7; l++) {
        dw_conv[filter_dim][k][l] = double(convolution(28, 28, img, k + 1, l - 2, dw_max[filter_dim])) / 1024.0;
      }
    }
  }
  return 0;
...
int filter_ite(unsigned char img1[1120], int weight[49], int filter_dim) {
  const int filter_size = 7;
  double conv_layer1 = 0;
  for (int i = 0; i < 28; i++) {
    for (int j = 0; j < 28; j++) {
      conv_layer1 = double(convolution(filter_size, filter_size, img1, i + 1, j - 2, weight)) / 1024.0;
