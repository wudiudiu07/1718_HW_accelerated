...
void forward_pass(unsigned char img[][32]) {
  ...
  for (int filter_dim = 0; filter_dim < 5; filter_dim++) {
    ofstream input_file("input");
    ofstream output_file("output");
    for (int k = 0; k < filter_size; k++) {
      for (int l = 0; l < filter_size; l++) {
        input_file << (int)((conv_w[filter_dim][k][l]) * pow(2, 10)) << " ";
      }
    }
    input_file << "\n";
    ...
    for (int i = 0; i < 28; i++) {
      for (int j = 0; j < 28; j++) {
        ...
        for (int k = 0; k < filter_size; k++) {
          for (int l = 0; l < filter_size; l++) {
            input_file << (int)(img[i + k + 1][j + l - 2]) << " ";
            conv_layer[filter_dim][i][j] +=
                (int)(img[i + k + 1][j + l - 2]) * (int)((conv_w[filter_dim][k][l]) * pow(2, 10));
          }
        }
        input_file << "\n";
        output_file << (int)conv_layer[filter_dim][i][j] << " ";
        sig_layer[filter_dim][i][j] =
            sigmoid(conv_layer[filter_dim][i][j] + conv_b[filter_dim][i][j]);
      }
    }
    input_file.close();
    output_file.close();
  ...
  std::cout << "Convolution Operation + Sigmoid Activation time: " << duration
            << '\n';
  ...
  std::cout << "Dense Layer 2: " << duration << '\n';
  ...
  std::cout << "Softmax Output: " << duration << '\n';
}
